...
class Proxy:
    ...
    async def _handle_change(self, request: Request, zone_id: str) -> Response:
        ...
        try:
            ...
            return Response(content="<ErrorResponse><Error><Code>InvalidInput</Code><Message>Failed to parse request XML</Message></Error></ErrorResponse>", status_code=400, media_type="application/xml")
        if not changes:
            return Response(content="<ErrorResponse><Error><Code>InvalidInput</Code><Message>No changes in request</Message></Error></ErrorResponse>", status_code=400, media_type="application/xml")
    ...
    async def _direct_change(self, zone_id: str, changes: list[dict]) -> Response:
        try:
            ...
            return Response(content=build_change_response(real_id.split("/")[-1]), status_code=200, media_type="application/xml")
        except Exception:
            logger.exception("Direct Route53 call failed for zone %s", zone_id)
            return Response(content="<ErrorResponse><Error><Code>ServiceUnavailable</Code><Message>Route53 call failed</Message></Error></ErrorResponse>", status_code=503, media_type="application/xml")
    ...
    async def _handle_get_change(self, change_id: str) -> Response:
        ...
        try:
            ...
            return Response(content="<ErrorResponse><Error><Code>ServiceUnavailable</Code><Message>GetChange failed</Message></Error></ErrorResponse>", status_code=503, media_type="application/xml")
    ...
    async def _handle_passthrough(self, request: Request) -> Response:
        ...
        return Response(content="<ErrorResponse><Error><Code>NotImplemented</Code><Message>This Route53 operation is not proxied</Message></Error></ErrorResponse>", status_code=501, media_type="application/xml")
