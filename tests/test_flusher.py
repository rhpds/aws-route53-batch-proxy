...
async def flusher(redis_client, batcher):
    mock_r53 = MagicMock()
    mock_r53.change_resource_record_sets = MagicMock(return_value={"Id": "/change/REAL123", "Status": "PENDING"})
...
class TestFlushOnce:
    ...
    async def test_maps_change_ids_after_flush(self, flusher, batcher):
        await batcher.enqueue(
            "Z1", [{"action": "DELETE", "name": "a.example.com.", "type": "A", "values": ["1.1.1.1"]}], "BATCH-syn1"
        )
        await flusher.flush_once()
